...
pub enum ModeEvent {
    ...
    ModeChanged { from: InteractionMode, to: InteractionMode },
    InputProcessed { mode: InteractionMode, action: Action },
}
...
impl ModeManager {
    ...
    pub fn toggle_speed(&self) {
        if let Ok(mut fast_mode) = self.fast_mode.lock() {
            *fast_mode = !*fast_mode;
            info!("Speed mode toggled to: {}", if *fast_mode { "fast" } else { "normal" });
...
impl ModeController for ModeManager {
    async fn activate_mode(&mut self, mode: InteractionMode) -> Result<()> {
        ...
        if let Some(prev) = previous_mode.clone() {
            ...
            self.send_event(ModeEvent::ModeChanged { from: prev, to: mode.clone() });
        } else {
...
mod tests {
    ...
    use crate::models::{MouseButton, Position, AnimationType};
