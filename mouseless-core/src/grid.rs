use crate::{Position, ScreenBounds, GridConfig, MouselessError, Result};
...
impl GridManager {
        ...
        for row in 0..self.config.rows {
            for col in 0..self.config.columns {
                if key_index >= key_combinations.len() {
                    return Err(MouselessError::SystemError(
    ...
    fn generate_key_combinations(&self) -> Result<Vec<String>> {
        ...
        if combinations.len() < total_cells {
            return Err(MouselessError::SystemError(
                std::io::Error::new(std::io::ErrorKind::InvalidInput, 
                                   format!("Cannot generate enough key combinations for {}x{} grid", 
    ...
    pub fn get_cell_by_keys(&self, key_combination: &str) -> Option<&GridCell> {
        self.key_to_cell.get(key_combination)
...
mod tests {
    ...
    fn test_key_combination_patterns() {
        ...
        for cell in cells {
            assert_eq!(cell.key_combination.len(), 2, 
            ...
            assert!(
                ...
                "First character '{}' should be from home row", first_char
            );
            ...
            assert!(
                ...
                "Second character '{}' should be from top row", second_char
            );
        }
    }
    ...
    fn test_comprehensive_grid_functionality() {
        ...
        for (rows, columns, expected_cells) in test_cases {
            ...
            assert_eq!(cells.len(), expected_cells, 
                      "{}x{} grid should have {} cells", rows, columns, expected_cells);
            
            // Test that all key combinations are unique
            let mut key_combinations = std::collections::HashSet::new();
            for cell in cells {
                assert!(key_combinations.insert(cell.key_combination.clone()),
                       "Duplicate key combination: {}", cell.key_combination);
            }
