...
use mouseless_core::{
    ...
    models::{Position, AnimationType, MovementSpeed},
};
...
fn benchmark_animation_interpolation(c: &mut Criterion) {
    ...
    for speed in [MovementSpeed::Slow, MovementSpeed::Normal, MovementSpeed::Fast] {
        for animation_type in [AnimationType::Linear, AnimationType::Smooth, AnimationType::Bounce] {
...
fn benchmark_single_step_performance(c: &mut Criterion) {
    ...
    group.bench_function("single_step_fast_smooth", |b| {
        b.iter(|| {
            ...
            assert!(step_duration.as_millis() < 1, "Step calculation took too long: {}ms", step_duration.as_millis());
        })
    });
...
fn benchmark_full_animation_sequence(c: &mut Criterion) {
    ...
    group.bench_function("fast_linear_full_sequence", |b| {
        b.iter(|| {
            ...
            assert!(sequence_duration.as_millis() < 5, "Full sequence generation took too long: {}ms", sequence_duration.as_millis());
        })
    });
...
fn benchmark_performance_requirements(c: &mut Criterion) {
    ...
    group.bench_function("critical_path_sub_10ms", |b| {
        b.iter(|| {
            ...
            assert!(critical_duration.as_millis() < 1, "Critical path took {}ms, must be under 10ms", critical_duration.as_millis());
        })
    });
