...
use crate::{
    error::Result,
    models::{KeyInput, Action, Position, Area, InteractionMode, AnimationType, ScreenBounds},
    traits::KeyBindings,
};
...
impl AreaMode {
    ...
    fn calculate_areas(&mut self) {
        ...
        debug!("Calculated {} areas for screen {}x{}", 
               self.areas.len(), self.screen_width, self.screen_height);
    }
    
    /// Process input for area mode
    pub fn process_input(&mut self, input: KeyInput, _key_bindings: &KeyBindings) -> Result<Action> {
        if !self.is_active {
        ...
        // Handle exit keys
        if key_char == ' ' || input.key == '\x1b' { // \x1b is escape
        ...
        match self.first_key {
            None => {
                ...
                debug!("First area key pressed: {}, waiting for potential combination", key_char.to_uppercase());
            ...
            Some(first_key) => {
                ...
                debug!("Area key combination: {} + {}", 
                       first_key.to_uppercase(), key_char.to_uppercase());
                ...
                if let Some(position) = combination_position {
                    ...
                    if let Some(area) = self.areas.get(&key_char) {
                        ...
                        debug!("Invalid combination, moving to second key area: {}", 
            ...
            if let Some(first_time) = self.first_key_time {
                ...
                if now.duration_since(first_time).as_millis() > 400 {
                    ...
                    if let Some(area) = self.areas.get(&first_key) {
                        let center = area.center.clone();
                        debug!("Timeout reached, moving to area {} center: {:?}", 
                               first_key.to_uppercase(), center);
                        self.reset_combination_state();
                        return Ok(Action::MoveCursor(center, AnimationType::Smooth));
                    }
                }
    ...
    fn calculate_combination_position(&self, first_key: char, second_key: char) -> Option<Position> {
        ...
        let intersection_pos = if col1 == col2 {
            ...
            Position { x, y, screen_id: None }
        };
        
        debug!("Combination {} + {} -> grid positions ({},{}) + ({},{}) -> intersection {:?}",
               first_key.to_uppercase(), second_key.to_uppercase(),
               col1, row1, col2, row2, intersection_pos);
    ...
    fn get_area_grid_position(&self, key: char) -> Option<(usize, usize)> {
        match key {
            'q' => Some((0, 0)), 'w' => Some((1, 0)), 'e' => Some((2, 0)), // Top row
            'a' => Some((0, 1)), 's' => Some((1, 1)), 'd' => Some((2, 1)), // Middle row
            'z' => Some((0, 2)), 'x' => Some((1, 2)), 'c' => Some((2, 2)), // Bottom row
            _ => None,
        }
    }
}
...
mod tests {
    ...
    fn test_area_input_processing() {
        ...
        let action = mode.process_input(create_test_input('q'), &bindings).unwrap();
        ...
        mode.first_key_time = Some(std::time::Instant::now() - std::time::Duration::from_millis(500));
    ...
    fn test_combination_input_processing() {
        ...
        let action = mode.process_input(create_test_input('e'), &bindings).unwrap();
        match action {
    ...
    fn test_exit_keys() {
        ...
        let action = mode.process_input(create_test_input('\x1b'), &bindings).unwrap();
        assert_eq!(action, Action::DeactivateMode);
    }
}